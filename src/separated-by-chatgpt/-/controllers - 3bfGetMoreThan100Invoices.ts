...
export async function invoicesToPayByContact(req: Request, res: Response) {
  ...
  try {
    ...
    try {
      ...
      console.log("1: " + invoices.response.headers)
      console.log("2: " + JSON.stringify(invoices.response.headers))
...
export async function invoiceToPay(req: Request, res: Response) {
  ...
  try {
    ...
    try {
      ...
      var invoicesInfo = invoices.body.invoices.map(item => [item.type, item.total, item.payments, item.amountDue])

      res.json({
        consentUrl: await xero.buildConsentUrl(),
        invoiceInfo: invoicesInfo[0],
      });
...
export async function contactsTransactions(req: Request, res: Response) {
  try {
    // const tenantId = "33b67fb1-e1c6-414c-bc39-c4305af2146f";
    const tenantId = "89a7e972-1cbb-441a-a98d-00f101c5dd8f"

    // MAKE FUNCTION TO GETLL ALL GROUPS AS DROPLIST with default of Spend
    /// DON'T USE where, fetch ALL
    // THEN, IT POPULATE THE NAMES OF SELECTED GROUP

    const where = `Name == "Spend"`; // Filter contact groups by name
    const order = ''; // Add sorting if necessary, e.g. 'Name ASC'

    const allContactGroups = await xero.accountingApi.getContactGroups(tenantId, where, order);

    const allContactGroups2 = await xero.accountingApi.getContactGroup(tenantId, allContactGroups.body.contactGroups[0].contactGroupID);
    
    // Group Spend- 0d620928-3d75-4066-8366-371943778340
    res.json({
      consentUrl: await xero.buildConsentUrl(),
      contactGroupID: allContactGroups.body.contactGroups[0].contactGroupID,
      contacts: allContactGroups2.body.contactGroups[0].contacts
    });
  } catch (e) {
    console.error(e);
    res.status(res.statusCode);
