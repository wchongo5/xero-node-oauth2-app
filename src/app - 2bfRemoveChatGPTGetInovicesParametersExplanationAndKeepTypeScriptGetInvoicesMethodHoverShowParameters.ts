      ...
      try {
        ...
        try {
          ...
          const invoices = await xero.accountingApi.getInvoices(
            ...
            // req.session.activeTenant.tenantId: This is the ID of the tenant (or organization) for which you want to retrieve invoices.


            //getInvoices(xeroTenantId: string, ifModifiedSince?: Date, where?: string, order?: string, iDs?: string[], invoiceNumbers?: string[], contactIDs?: string[], statuses?: string[], page?: number, includeArchived?: boolean, createdByMyApp?: boolean, unitdp?: number, summaryOnly?: boolean, options?: { headers: { [name: string]: string; }; }): Promise<{ response: IncomingMessage; body: Invoices; }>
            
            new Date("2022/01/01"), // ifModifiedSince?: Date
            
            
            //undefined, // new Date("2023/01/01"), //new Date(2018) Exige inclusão de Mês e dia assim new Date(2018, 1, 1), // new Date(2018): This specifies the start date for the date range of invoices to retrieve. In this case, it's set to January 1, 2018.
            // 🔴🔴🔴 The reason why the code below gives invoice issued in January 12, 2021 while the method receives date that starts in January 1, 2022 is likely due to the fact that the invoices retrieved by the Xero API are not filtered by the date the invoice was issued but rather by the date the invoice was created or modified in the Xero system.
            'Type=="ACCREC" && InvoiceDate>=DateTime(2022, 1, 1)', // where?: string
            
            //'Type=="ACCREC"', // 'Type=="ACCREC"': This is a filter to retrieve only invoices of the type "Accounts Receivable".
            
            undefined, // order?: string // 'reference DESC'
            
            // ChatGPT - 'reference DESC': This is the order in which the results will be returned. In this case, the invoices will be sorted in descending order by their reference field.
            // 🔴🔴🔴 Any parameter that is empty for some invoices, will raise undefined of invoices like 'reference DESC'
            
            undefined, // iDs?: string[]
            
            // ChatGPT - undefined: This parameter specifies which page of results to return. Since it's set to undefined, the first page of results will be returned.
            
            undefined, // invoiceNumbers?: string[]
            
            // ChatGPT - undefined: This parameter specifies the number of results to return per page. Since it's set to undefined, the default number of results per page will be used.

            undefined, // contactIDs?: string[]
            
            // ChatGPT - undefined: This parameter specifies the currency code to filter by. Since it's set to undefined, no currency filter will be applied.

            ['AUTHORISED'], // statuses?: string[] // ['AUTHORISED', 'PAID', 'DRAFT']
            
            // ChatGPT - ['AUTHORISED', 'PAID', 'DRAFT']: This parameter specifies the status of invoices to retrieve. In this case, invoices with a status of "PAID" or "DRAFT" will be returned. // XD5 Noted in Notes — DRAFT, SUBMITTED, AUTHORISED (Once an invoice is fully paid invoice the status will change to PAID). Note that if invoice is partially paid is AUTHORISED

            0, // page?: number
            
            // ChatGPT - 0: This parameter specifies the minimum total amount for invoices to retrieve. In this case, invoices with a total amount greater than or equal to zero will be returned.
            
            false, // includeArchived?: boolean
            
            // ChatGPT - true: This parameter specifies whether to include an If-Modified-Since header in the request. If set to true, the server will only return invoices that have been modified since the specified date (not included in this example).


            false, // createdByMyApp?: boolean
            
            // ChatGPT - false: This parameter specifies whether to include an If-None-Match header in the request. If set to true, the server will only return invoices that have a different ETag value than the one specified (not included in this example).
            ...
            // ChatGPT - 4: This parameter specifies the maximum number of results to return. In this case, a maximum of 4 invoices will be returned.
            // It returns 100 invices, even if you increase this number
            // 🔴 How to include get more than 100 invoices.
            
            true, // summaryOnly?: boolean
            
            // ChatGPT - true: This parameter specifies whether to include a Content-Type header in the request. If set to true, the request will include a Content-Type header with the value application/json.
            ...
            // ChatGPT - { headers: { 'contentType': 'application/json' } }: This is an optional parameter that specifies additional headers to include in the request. In this case, it includes a Content-Type header with the value application/json.
          )

          console.log("THANKS GOD - Y")

          console.log(invoices.body.invoices[0])
          var invoicesContacts = invoices.body.invoices.map(item => item.contact.name)
