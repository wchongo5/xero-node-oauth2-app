...
import { submitBankTransfer } from '../features/payments/paymentsSlice';
import "./Invoices.css";


// 🔴🔴🔴 REPLACE HARD CODED WITH THIS REDUX
...
const Transfers = () => {
  ...
  // const transferData = useSelector((state) => state.payments.transferData);
  const { transferData, loading, error } = useSelector((state) => state.payments);
  ...
  useEffect(() => {
    if (selectedTenantId) {
      fetchAccountsBank(selectedTenantId);
    }
  }, [selectedTenantId]);
  ...
  const fetchAccountsBank = async (tenantId) => {
    try {
      const response = await fetch(
        `http://localhost:3000/accounts-bank?tenantId=${tenantId}`
      );
   ...
   <div>
      ...
      {!loading && transferData && (
        <div className="payments-control">
          <div>
            <>
              <p>Valor transferido: {formatNumber(transferData.transferAmount)}</p>
            </>
          </div>
        </div>
      )}
