...
const Invoices = () => {
  ...
  useEffect(() => {
    if (contactsToPay.length > 0) {
      ...
      // If contactsToPay is empty, force handleCustomerChange to update customer name
      handleCustomerChange({ target: { value: null } });
    }
  }, [selectedInvoiceType, contactsToPay]);
  ...
  const handleCustomerChange = async (event) => {
    ...
    setSelectedInvoice(contactId);

    dispatch(fetchInvoicesToPay({ tenantId: selectedTenantId, contactId }))
      .unwrap()
      .then((data) => {
        const fetchedInvoicesToPay = data.invoices.map((invoice) => [invoice[0], invoice[1]]);
        setInvoicesToPay(fetchedInvoicesToPay);

        if (fetchedInvoicesToPay.length) {
          const defaultInvoiceId = fetchedInvoicesToPay[0][0];
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // Update invoice details
      if (selectedInvoice) {
        dispatch(fetchInvoiceDetails({ tenantId: selectedTenantId, invoiceId: selectedInvoice }))
          .unwrap()
          .then((data) => {
            setInvoiceDetails(data.invoiceInfo);
          });
