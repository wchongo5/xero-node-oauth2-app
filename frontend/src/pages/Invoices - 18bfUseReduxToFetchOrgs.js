...
const Invoices = () => {
  ...
  const fetchContactsToPay = async (tenantId, startDate, invoiceType) => {
    try {
      const response = await fetch("http://localhost:3000/contacts-to-pay", {
        ...
        body: JSON.stringify({ tenantId, startDate, invoiceType }),
      });
      const data = await response.json();
   ...
   <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-control-finance">
          ...
          <div>
            <div>Total: {formatNumber(invoiceDetails[0])}</div>
            <div>
              Pagamentos: {formatNumber(sumPayments(invoiceDetails[1]))}
              {invoiceDetails[1]
            ...
            <div>Remanescente: {formatNumber(invoiceDetails[2])}</div>
          </div>
        </div>
      )}
